...
import { airports, fbos, aircraft, charter, flightSchools, fleet, activity } from './api-client.js';

export function registerResources(server: McpServer): void {
  // API Overview resource
  server.resource(
    'api-overview',
    'openavdb://api/overview',
    async (uri) => {
      const overview = {
        name: 'OpenAvDB API',
        version: '2.0.0',
  ...
  server.resource(
    'top-airports-california',
    'openavdb://airports/state/CA',
    async (uri) => {
      try {
  ...
  server.resource(
    'top-airports-florida',
    'openavdb://airports/state/FL',
    async (uri) => {
      try {
  ...
  server.resource(
    'aircraft-cessna',
    'openavdb://aircraft/manufacturer/CESSNA',
    async (uri) => {
      try {
