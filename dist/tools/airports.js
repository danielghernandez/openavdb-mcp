...
export function registerAirportTools(server) {
    // Search airports
    server.tool('search_airports', 'Search for US airports by state, city, runway length, FBO count, or control tower status. Returns a list of airports matching the criteria.', {
    ...
    // Get airport details
    server.tool('get_airport', 'Get detailed information about a specific airport by ICAO code. Can optionally include FBOs, fleet, and activity data.', {
    ...
    server.tool('get_airport_fbos', 'Get all Fixed Base Operators (FBOs) at a specific airport. FBOs provide services like fuel, hangars, and ground handling.', {
    ...
    // Batch get airports
    server.tool('get_airports_batch', 'Get multiple airports by their ICAO codes in a single request. Efficient for comparing airports.', {
